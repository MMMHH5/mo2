                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {instructors?.map((instructor) => (
                        <Link
                            ...
                            className="bg-brand-navy-dark rounded-3xl border border-white/10 shadow-sm hover:shadow-xl transition-all p-8 flex flex-col items-center text-center group"
                        >
                            ...
                            <p className="font-black text-white text-lg mb-2 break-all" dir="ltr">{instructor.email}</p>
                            <p className="text-sm text-gray-400 font-semibold mb-6">
                                {isAr ? 'انضم في' : 'Joined'} {formatDate(instructor.joinedAt, { locale })}
                            </p>
                            <div className="flex items-center gap-3 w-full">
                                <div className="flex-1 bg-white/5 rounded-2xl py-3 text-center">
                                    <p className="text-xl font-black text-white">{formatNumber(instructor.courseCount, locale)}</p>
                                ...
                                <div className="flex-1 bg-white/5 rounded-2xl py-3 text-center">
                                    <p className="text-xl font-black text-white">{formatNumber(instructor.openingCount, locale)}</p>
                                    <p className="text-xs font-bold text-gray-400 flex items-center justify-center gap-1">
                    ...
                    {instructors?.length === 0 && (
                        <div className="col-span-full py-16 text-center bg-brand-navy-dark rounded-3xl border border-white/10 text-gray-400 text-lg">
